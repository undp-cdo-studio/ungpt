...
import { TranslationObject, loadTranslation } from './loadTranslation';
...
const getTranslationValue = (
  ...
  // Get the first key string from the keys array and assign it to "key".
  const key: string = keys.shift() || '';

  // Return key if translation for it doesn't exist
  if (!translation[key]) {
    return key;
  }
